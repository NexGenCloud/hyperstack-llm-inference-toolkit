...
async def test_guided_choice_chat(
    api_client: openai.AsyncOpenAI, guided_decoding_backend: str
):
    messages = [
        {"role": "system", "content": "You are a helpful assistant"},
        {
    ...
    chat_completion1 = await api_client.chat.completions.create(
        model=MODEL_NAME,
        messages=messages,
        max_tokens=10,
        extra_body=dict(
            guided_choice=TEST_CHOICE, guided_decoding_backend=guided_decoding_backend
        ),
    )
    choice1 = chat_completion1.choices[0].message.content
    assert choice1 in TEST_CHOICE

    messages.append({"role": "assistant", "content": choice1})
    messages.append({"role": "user", "content": "Wrong choice, I like Python more."})
    chat_completion2 = await api_client.chat.completions.create(
        model=MODEL_NAME,
        messages=messages,
        max_tokens=10,
        extra_body=dict(
    ...
    choice2 = chat_completion2.choices[0].message.content
...
async def test_guided_json_chat(
    ...
    chat_completion = await api_client.chat.completions.create(
        ...
        max_tokens=1000,
        extra_body=dict(
            guided_json=TEST_SCHEMA, guided_decoding_backend=guided_decoding_backend
        ),
    )
    message = chat_completion.choices[0].message
    ...
    messages.append(
        {
            "role": "user",
            "content": "Give me a different one with a different name and age",
        }
    )
    chat_completion = await api_client.chat.completions.create(
        model=MODEL_NAME,
...
async def test_guided_regex_chat(
    ...
    chat_completion = await api_client.chat.completions.create(
        ...
        max_tokens=20,
        extra_body=dict(
            guided_regex=TEST_REGEX, guided_decoding_backend=guided_decoding_backend
        ),
    )
    ...
    messages.append(
        {"role": "user", "content": "Give me a different ip than the first one"}
    )
    chat_completion = await api_client.chat.completions.create(
        model=MODEL_NAME,
        messages=messages,
        ...
        messages=[
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "What is 1+1? Please only respond with the number and no other text.",
                    }
                ],
            }
        ],
        ...
        messages=[
            {
                "role": "user",
                "content": (
                    "What is 1+1? please respond with a JSON object containing the result key and "
